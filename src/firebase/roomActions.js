...
export async function playCard(roomCode, playerId, card, chosenColor = null) {
  const roomRef = doc(db, 'rooms', roomCode);
  ...
  await runTransaction(db, async (transaction) => {
    ...
    const nextIndex = getNextPlayerIndex(
      room.currentPlayerIndex, newDirection, room.playerOrder.length, steps
    );
    ...
    transaction.update(roomRef, {
      ...
      winnerId: newHand.length === 0 ? playerId : null,
    });
  });
}
